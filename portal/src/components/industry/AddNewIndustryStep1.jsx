...
import { 
  ...
  Globe, ArrowRight, Bookmark, AlertCircle
} from 'lucide-react';
...
export default function AddNewIndustryStep1({ onNext, onCancel, formData, updateFormData, externalErrors = {} }) {
  ...
  const inputClass = (field) => `${inputBase} ${externalErrors[field] ? 'border-rose-400 bg-rose-50/30' : 'border-slate-200'}`;
  const selectClass = (field) => `w-full appearance-none bg-white border rounded-lg px-3.5 py-2.5 pr-10 text-sm text-slate-700 outline-none focus:border-indigo-500 shadow-sm cursor-pointer transition-colors ${externalErrors[field] ? 'border-rose-400 bg-rose-50/30' : 'border-slate-200'}`;
          ...
          <div className="grid grid-cols-2 gap-6">
            ...
            <div className="space-y-1.5">
              ...
              <input 
                ...
                className={inputClass('industryName')}
              />
              {externalErrors.industryName && (
                <p className="flex items-center gap-1 text-[11px] text-rose-500 font-medium">
                  <AlertCircle className="w-3 h-3 shrink-0" />{externalErrors.industryName}
                </p>
              )}
            </div>
            ...
            <div className="space-y-1.5">
              ...
              {externalErrors.industryType && (
                <p className="flex items-center gap-1 text-[11px] text-rose-500 font-medium">
                  <AlertCircle className="w-3 h-3 shrink-0" />{externalErrors.industryType}
                </p>
              )}
            </div>
            ...
            <div className="space-y-1.5">
              ...
              <input 
                ...
                className={inputClass('industryCode')}
              />
            </div>
