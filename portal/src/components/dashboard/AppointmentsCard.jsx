...
export default function AppointmentsCard({ stats, loading }) {
  const items = [
    { label: 'Scheduled', count: stats?.scheduled ?? 0, icon: <Calendar className="text-blue-500" size={20} />, bg: 'bg-blue-50' },
    { label: 'Completed', count: stats?.completed ?? 0, icon: <UserCheck className="text-emerald-500" size={20} />, bg: 'bg-emerald-50' },
    { label: 'Cancelled', count: stats?.cancelled ?? 0, icon: <UserX className="text-rose-500" size={20} />, bg: 'bg-rose-50' },
  ];
        ...
        <div className="grid grid-cols-3 gap-3">
          {items.map((item, idx) => (
            <div key={idx} className="flex flex-col items-center text-center p-3 rounded-xl hover:bg-slate-50 transition border border-transparent hover:border-slate-200">
              ...
              <span className="text-2xl font-black text-slate-900 mb-1">
                {loading ? '...' : item.count}
              </span>
